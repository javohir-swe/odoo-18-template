...
RUN python3 -m pip install --force-reinstall --no-cache-dir pip setuptools wheel
RUN pip install --no-cache-dir --upgrade "setuptools<58"
RUN pip install --no-cache-dir --use-pep517 --config-settings editable_mode=compat -r /odoo/base_requirements.txt --ignore-installed
RUN pip install --no-cache-dir --use-pep517 "Adyen==7.1.1"
RUN pip install --no-cache-dir --use-pep517 -r /odoo/extra_requirements.txt --ignore-installed
