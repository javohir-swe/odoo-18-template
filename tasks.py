...
ODOO_VERSION = float(os.environ.get("ODOO_VERSION", 18.0))
DOCKER_COMPOSE_CMD = (
...
def write_code_workspace_file(c, cw_path=None):
    ...
    `*.code-workspace` file found inside the current directory.
    If none is found, it will default to `$(basename $PWD).code-workspace`.

    If you define it manually, remember to use the same prefix and suffix if you
    want it git-ignored by default.
    Example: `--cw-path my-custom-name.code-workspace`
    """
    root_name = f"{PROJECT_ROOT.name}"
    root_var = "${workspaceFolder:%s}" % root_name
    if not cw_path:
        try:
            cw_path = next(PROJECT_ROOT.glob("*.code-workspace"))
    ...
    cw_config["settings"].update(
        {
            "python.autoComplete.extraPaths": [f"{str(SRC_PATH)}/"],
            # "python.formatting.provider": "none",
            # "python.linting.flake8Enabled": True,
            # "python.linting.ignorePatterns": [f"{str(SRC_PATH)}/**/*.py"],
            # "python.linting.pylintArgs": [
            #     f"--init-hook=\"import sys;sys.path.append('{str(SRC_PATH)}/')\"",
    ...
    firefox_configuration = {
        ...
        "reloadOnChange": {
            "watch": f"{root_var}/src/local/**/*.{'{js,css,scss,less}'}"
        },
        "skipFiles": ["**/lib/**"],
        "pathMappings": [],
    }
    ...
    cw_config["tasks"] = {
        ...
        "tasks": [
            ...
            },
            {
                ...
                "type": "process",
                "command": "invoke",
                "args": ["install", "--cur-file", "${file}", "update"],
                "presentation": {
                    "echo": True,
                    "reveal": "always",
                    "focus": True,
                ...
                "problemMatcher": [],
                "options": {
                    "statusbar": {"label": "$(symbol-property) Update modules"}
                },
            },
            {
...
def git_aggregate(c):
    ...
    Executes git-aggregator from within container.
    """
    with c.cd(str(PROJECT_ROOT)):
        c.run(
            DOCKER_COMPOSE_CMD + " --file devel.yaml run --rm -T odoo",
            env=UID_ENV,
        )
    write_code_workspace_file(c)
    ...
    with tempfile.NamedTemporaryFile(
        ...
        with c.cd(str(PROJECT_ROOT)):
            result = c.run(
                ...
                env=dict(
                    UID_ENV,
                    DEBUGPY_ENABLE=str(int(debugpy)),
                ),
            )
            if not (
...
@task(
    help={
        "module_name": "Name of the new module to create.",
    },
)
def new_module(
    c,
    ...
    with c.cd(str(PROJECT_ROOT)):
        c.run(
            DOCKER_COMPOSE_CMD
            + f" run --rm odoo odoo scaffold {module_name} /odoo/local-src",
            env=UID_ENV,
            pty=True,
        )
...
def update(
    ...
    cmd = DOCKER_COMPOSE_CMD + f" run --rm odoo click-odoo-update"
    with c.cd(str(PROJECT_ROOT)):
        c.run(
            cmd,
            env=UID_ENV,
...
@task(
    help={
        "dbname": "The DB that will be DESTROYED and recreated. Default: 'odoodb'.",
...
def resetdb(
    ...
    with c.cd(str(PROJECT_ROOT)):
        ...
        _run = DOCKER_COMPOSE_CMD + " run --rm odoo"
        c.run(
        ...
        c.run(
            f"{_run} click-odoo-initdb -n {dbname} -m {modules} {params}",
            env=UID_ENV,
            pty=True,
        )
...
def preparedb(c):
    ...
    if ODOO_VERSION < 11:
        raise exceptions.PlatformError(
            "The preparedb script is not available for environments bellow v11."
        )
    with c.cd(str(PROJECT_ROOT)):
        c.run(
            DOCKER_COMPOSE_CMD + " run --rm odoo odoo preparedb",
            env=UID_ENV,
            pty=True,
        )
...
@task(
    help={
        "source_db": "The source DB name. Default: 'odoodb'.",
...
@task(
    help={
        ...
        "destination_db": "The destination DB name. Default: 'odoodb'",
    },
)
def restore_snapshot(
    c,
    snapshot_name=None,
    destination_db="odoodb",
):
